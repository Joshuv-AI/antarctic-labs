...
console.log(`Cloud srcDoc with markers: ${cloudSrc.length} chars`);

// Reuse the same wrapper injections (gsap stub + focusStyles + focusScript + probe)
...
(function() {
  ...
  window.addEventListener('error', function(e) {
    record('window.error', { message: e.message, lineno: e.lineno, stack: (e.error && e.error.stack || '').slice(0, 600) });
  });
...
fs.writeFileSync('/tmp/cloud-markers-final.html', patched);
console.log(`Final patched: ${patched.length} chars`);
...
(async () => {
  ...
  events.forEach((e) => {
    console.log(`[${e.t.toFixed(0)}ms] ${e.stage}: ${JSON.stringify(e.data).slice(0, 400)}`);
  });

  await page.screenshot({ path: '/tmp/cloud-markers-screenshot.png' });
  console.log('saved /tmp/cloud-markers-screenshot.png');
